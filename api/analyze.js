...
const saifSystemPrompt = `You are a compliance architecture analyst specializing in mapping external regulatory requirements to internal AI governance frameworks.
...
export default async function handler(req, res) {
  ...
  if (!prompts[mode]) {
    return res.status(400).json({ error: 'Invalid mode. Must be "translator", "riskScorer", or "saif"' });
  }
